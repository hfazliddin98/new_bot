#!/usr/bin/env python
"""
Yetkazib berish soatlari bilan yangilangan Telegram bot
"""
import os
import sys
import django
...
def start_command(message):
    ...
    try:
        ...
        btn5 = types.KeyboardButton('📞 Aloqa')
...
def zone_callback(call):
    ...
    try:
        ...
        if not zone.is_working_now():
            text = f"❌ *{zone.name}* hozir ishlamaydi\n\n"
            text += f"🕐 Ish soatlari: {zone.get_working_hours_display()}\n"
            text += f"🔄 Keyingi ochilish: Ertaga {zone.working_hours_start.strftime('%H:%M')}\n\n"
            text += "Iltimos, ish soatlarida qaytadan urinib ko'ring."
...
def dormitory_callback(call):
    ...
    try:
        ...
        text += f"🕐 Kutilayotgan yetkazib berish: {expected_delivery.strftime('%H:%M')}\n\n"
...
⚠️ *Diqqat:* Buyurtma faqat ish soatlarida qabul qilinadi.
...
🏠 Yotoqxonalar ro'yxatini ko'rish uchun asosiy menyudan "🏠 Yotoqxonalar" tugmasini bosing."""
        ...
        hours_btn = types.InlineKeyboardButton('⏰ Ish soatlari', callback_data='show_hours')
        markup.add(dorms_btn)
        markup.add(hours_btn)
...
def show_dormitories_callback(call):
    ...
    try:
        ...
        text = "🏠 *Yotoqxonani tanlang:*\n\n"
        text += f"🕐 Hozirgi vaqt: {timezone.now().strftime('%H:%M')}\n\n"
...
def handle_user_input(message):
    ...
    try:
        ...
        if state_data['state'] == UserState.SELECT_ROOM:
            ...
            safe_send_message(message.chat.id, text, parse_mode='Markdown')
            logger.info(f"Buyurtma yaratildi: #{order.id} - {expected_delivery.strftime('%H:%M')}")
...
def main():
    ...
    print("🤖 Yetkazib berish soatlari bilan telegram bot ishga tushmoqda...")
